...
function findUni(arr) {
  const unisWithUni = arr.filter(student => student.university.includes('Uni'));
  return unisWithUni;
}
...
console.log(findNames(zooAnimals));
...
*/
function animalNameList(arr) {
  const lowCaseAnimalNames = arr.map(animal => animal.animal_name.toLowerCase());
  return lowCaseAnimalNames;
}

console.log(animalNameList(zooAnimals));
...
function endangered(arr) {
  const lowPopulationAnimals = arr.filter(animal => animal.population < 5);
  return lowPopulationAnimals;
}

console.log(endangered(zooAnimals));
...
function totalAnimalPop(arr) {
  const populationTotal = Object.values(arr).reduce((acc, {population}) => acc + population, 0);
  return populationTotal;
}

console.log(totalAnimalPop(zooAnimals));
